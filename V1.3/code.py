...
def ensemble_model(datasplit,n_folds,flag):
    result=[]
    for i in range(n_folds):
        result.append(0)
    for i in range(len(datasplit)):
        ...
        for j in range(len(datasplit)):
            ...
            X = df.iloc[:,:-1]
            model.fit(X, y)    
        result[i] = calculate_statistics(datasplit[i],model)
    #print(precision)
...
class ui():
    ...
    def startreading(self):
        ...
        randomforest_result = ensemble_model(datasplit,int(n_folds),1)
        self.set_statistics(randomforest_result,1)
        GradientBoosting_result = ensemble_model(datasplit,int(n_folds),2)
        self.set_statistics(GradientBoosting_result,2)
        AdaBoost_result = ensemble_model(datasplit,int(n_folds),3)
        self.set_statistics(AdaBoost_result,3)
        Bagging_result = ensemble_model(datasplit,int(n_folds),4)
    ...
    def __init__(self,root):
        ...
        self.label_path = Label(root,text = "K-folds数(若不填写默认为3):",font=('黑体',12,'bold','italic'),anchor="w").grid(row = 3, column = 0)
